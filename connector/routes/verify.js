...
router.post('/', (req, res, next) => {
  ...
  let callMetadata = JSON.parse(fs.readFileSync(`${MEDIA_BASE}${req.body.CallSid}-${req.body.Attempt}-v.json`))
  debug(callMetadata);
  ...
  const cleanPhone = callMetadata.customParameters.caller.replace('+','');
  ...
  avClient.verify(cleanPhone, avClient.VoiceprintType.DIGIT, utterance, process.env.ARMORVOX_CHANNEL, null).then( (resp) => {
